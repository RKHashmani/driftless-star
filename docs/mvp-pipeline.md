...
The configured method in `stage5_post_processing.py` decides convergence. The two pressure methods compare the total pressure profile `P(rho)` in this pass's `transport_solution.h5` between its initial and final time slices. `resolve_pressure_convergence()` selects one of:
...
- `t_final`, implemented by `t_final_converged()`: a debugging criterion. It reports convergence when the solution's `final_time` has reached `[transport_solver].t_final`. It reads neither the pressure measures nor `pressure_rel_tol`.

The RMS method allows a change confined to a few radii to be averaged below the tolerance, while the pointwise method requires every radius to settle within it. Both pressure measures stay independent of the profile's radial resolution.
...
| `converged` | the profile has settled | the selected pressure-change measure is below `pressure_rel_tol`. Reported ahead of `horizon`, since a pass that both settled and ran out of time is better described as settled. Under `t_final` it is set when `final_time` reaches `[transport_solver].t_final`. |
| `horizon` | no transport time is left | the solution's `final_time` has reached `[transport_solver].t_final`. Because each pass resumes where the last one stopped, `t_final` is an absolute end time and not a per-pass window length, so there is nothing further to advance into. Raise `t_final` to keep evolving. This status cannot occur under the `t_final` method. That method reports the same condition as `converged`. |
| `continue` | none of the above | the loop runs another pass, up to `--max-iters`. |

If the transport solution has fewer than two distinct time slices, the pressure methods cannot assess convergence. The pass is then not converged, which is neither a halt nor an error.
